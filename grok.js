// grok.js (Content Script for Grok)
console.log('Grok content script loaded.'); // Keep this outside the guard to see how many times the file is initiated.
...
var grokContentScriptExecuted = grokContentScriptExecuted || false;

// Check if the script has already been executed. If the flag is true, exit immediately.
if (grokContentScriptExecuted) {
    console.warn('Grok content script already executed in this tab. Exiting core logic.');
} else {
    // Set the flag to true to indicate that the script is now running its core logic.
    grokContentScriptExecuted = true;
    console.log('Grok content script execution flag set. Running core logic.');
    ...
    // This will store the initial prompt sent to Grok when dual prompting starts.
    var savedLeftPrompt = null; // Stores the initial prompt from the control panel

    // Flag to indicate if Grok is currently processing a prompt
    var isProcessingGrokPrompt = false; // Initialize within the guard
    ...
    console.log('grok.js: pingInterval variable declared and interval started.');


    // --- Grok Button State Observer Class ---
    // Monitors the state of the AI chat interface buttons (idle, send, responding)
    class GrokButtonStateObserver {
         ...
         start() {
            if (this.observing) {
                console.log('grok.js: Grok Button Observer is already running');
                return;
            }

            console.log('grok.js: Grok Button Observer: Starting observer.');
            ...
            this.observer = new MutationObserver(mutations => {
                ...
                try {
                    ...
                    console.error('grok.js: Error during GrokButtonStateObserver check:', error);
            ...
            this.observing = true;
            console.log('grok.js: Grok button state observer started');
         }

         stop() {
            if (!this.observing) {
                console.log('grok.js: Observer is not running');
                return;
            }

            this.observer.disconnect();
            this.observing = false;
            console.log('grok.js: Grok button state observer stopped');
         }

         checkButtonState() {
             // --- IMPORTANT: These selectors are specific to Grok's current UI. ---
             // --- They are prone to breaking if Grok updates its front-end. ---
             // --- If the extension stops working, check these selectors first! ---

             // Use the specific selectors provided by the user for Grok
             const idleButton = document.querySelector('button[aria-label="Submit"][disabled]');
             const sendButton = document.querySelector('button[aria-label="Submit"]:not([disabled])');
             const stopButton = document.querySelector('button[aria-label="Stop model reponse"]'); // Note the typo "reponse" from user input
             ...
             if (isStopButtonVisible) {
                 ...
                 // console.warn('grok.js: ButtonStateObserver: No expected button visible. State remains:', this.currentState); // Keep this quiet
             ...
             if (newState !== this.currentState) {
                 console.log(`grok.js: Grok button state changed: ${this.currentState} -> ${newState}`);
    ...
    console.log('grok.js: GrokButtonStateObserver class declared.');
    // --- End Grok Button State Observer Class ---


    // Create an instance of the GrokButtonStateObserver, passing the handler function
    var grokButtonObserver = new GrokButtonStateObserver(handleGrokButtonStateChange);
    console.log('grok.js: GrokButtonStateObserver instance created.');


    // --- Response Handling Logic ---
    // Function to handle state changes reported by the GrokButtonStateObserver
    // This function now receives both the new and old state
    async function handleGrokButtonStateChange(newState, oldState) {
        console.log('grok.js: handleGrokButtonStateChange called with newState:', newState, 'oldState:', oldState);

        // We are interested when the state transitions from 'responding' to 'idle'
        if (oldState === 'responding' && newState === 'idle') {
            console.log("grok.js: State transitioned from 'responding' to 'idle'. Response is likely finished.");

            // Stop the observer immediately
            if (grokButtonObserver && grokButtonObserver.observing) {
                 grokButtonObserver.stop();
                 console.log("grok.js: Stopped GrokButtonStateObserver.");
            }
            ...
            console.log("grok.js: Attempting to extract response text after 100ms delay.");
            ...
            const responseElementSelector = '.message-bubble'; // Selector for Grok's message bubbles
            ...
            if (responseElements.length > 0) {
                ...
                console.log("grok.js: Extracted latest response text:", latestResponseText);
                console.log("grok.js: Latest response element:", lastResponseElement);
            } else {
                 console.warn(`grok.js: State is 'idle', but response element with selector "${responseElementSelector}" not found. This might be due to UI changes.`);
            }
            ...
            if (isDualPromptingActive) {
                console.log("grok.js: Dual prompting is active. Sending Grok response to background script.");
                try {
                    await chrome.runtime.sendMessage({
                        action: "grokResponseFinished", // Specific action for Grok
                        responseText: latestResponseText // Send the extracted text
                    });
                    console.log("grok.js: Response sent to background script");
                } catch (error) {
                    console.error("grok.js: Error sending response to background script:", error);
                }
            } else {
                console.log("grok.js: Dual prompting is NOT active. Skipping sending Grok response.");
            }
            // --- End Send Check ---

            // Reset the processing flag after handling the response
            isProcessingGrokPrompt = false;
            console.log("grok.js: isProcessingGrokPrompt set to false.");

        } else if (newState === 'responding') {
             console.log("grok.js: Button state is 'responding'. AI is generating response.");
             // We don't need to do anything specific here in this flow,
             // but the log confirms we detect this state.
        } else if (newState === 'send') {
             console.log("grok.js: Button state is 'send'. Input area is ready.");
             ...
             // (promptToInput is not used in the current Grok flow, but keeping this check for general structure)
             // if (promptToInput !== null) {
             //     console.log("grok.js: Button state is 'send' and promptToInput exists. Attempting to input prompt.");
             //     inputPromptAndSend(promptToInput);
             //     promptToInput = null; // Clear the prompt after attempting to input it
             // }
        } else if (newState === 'idle') {
            console.log("grok.js: Button state is 'idle'.");
    ...
    console.log('grok.js: handleGrokButtonStateChange function declared.');
    ...
    function waitForElement(selector, timeout = 15000) { // Increased default timeout slightly
        return new Promise((resolve, reject) => {
            console.log(`grok.js: waitForElement: Waiting for selector: ${selector}`); // Added log
            const element = document.querySelector(selector);
            if (element) {
                console.log(`grok.js: waitForElement: Found element immediately for selector: ${selector}`); // Added log
            ...
            const observer = new MutationObserver((mutations) => {
                const element = document.querySelector(selector);
                if (element) {
                    console.log(`grok.js: waitForElement: Found element via observer for selector: ${selector}`); // Added log
                    observer.disconnect();
                    resolve(element);
                }
            });
            ...
            if (timeout) {
                const timeoutId = setTimeout(() => {
                    observer.disconnect();
                    console.error(`grok.js: waitForElement: Timeout waiting for selector: ${selector}`); // Added log
                    reject(new Error(`grok.js: waitForElement: Timeout waiting for selector: ${selector}`));
                }, timeout);
    ...
    console.log('grok.js: waitForElement function declared.');
    ...
    function startResponseObserver() {
        ...
        // --- They are prone to breaking if Grok updates its front-end. ---
        // --- If the extension stops working, check these selectors first! ---
        // Need to identify a stable container for Grok's chat messages if needed for observation
        // For now, relying on button state observer is sufficient.
        console.warn('grok.js: Response observer is currently not implemented for Grok.');
        // Example selector if needed: const chatContainerSelector = '...';
        // waitForElement(chatContainerSelector)...
    }
    console.log('grok.js: startResponseObserver function declared.');
    ...
    function processNewMessage(messageNode) {
         console.warn('grok.js: processNewMessage is currently not implemented for Grok.');
    ...
    console.log('grok.js: processNewMessage function declared.');


    // Function to input the prompt text into the Grok UI and click send
    async function inputPromptAndSend(promptText) {
        console.log('grok.js: inputPromptAndSend called with prompt:', promptText);

        // --- Grok-Specific Selectors and Properties ---
        console.log("grok.js: Using Grok selectors.");
        const promptInputSelector = 'textarea[aria-label="Ask Grok anything"]'; // Selector for Grok's textarea input
        const sendButtonSelector = 'button[aria-label="Submit"]:not([disabled])'; // Selector for Grok's ENABLED send button
        const isContentEditable = false; // It's a textarea
        const sendButtonTimeout = 10000;
        // Grok's send button uses the disabled attribute, which our :not([disabled]) selector handles.


        // --- Check if the prompt is empty ---
        if (!promptText || promptText.trim() === '') {
            console.warn("grok.js: Prompt text is empty or whitespace only. Skipping input and send.");
            isProcessingGrokPrompt = false; // Reset processing flag
        ...
        try {
            console.log(`grok.js: inputPromptAndSend: Waiting for prompt input with selector: ${promptInputSelector}`); // Added log
            // Wait for the prompt input element (textarea) to be available
            const promptInput = await waitForElement(promptInputSelector, 20000); // Increased timeout to 20 seconds

            console.log('grok.js: Found prompt input. Attempting to input text.'); // This log confirms element was found
            console.log('grok.js: Prompt input element:', promptInput); // Log the actual element

            // Use a try-catch block for DOM manipulation
            try {
                 console.log('grok.js: Clearing existing content in prompt input field.'); // Added log
                 // Clear existing content for textarea
                 promptInput.value = '';
                 console.log('grok.js: Existing content cleared.'); // Added log

                 console.log('grok.js: Setting new prompt text.'); // Added log
                 // Set the new prompt text for textarea
                 promptInput.value = promptText;
                 console.log('grok.js: New prompt text set.'); // Added log

                 console.log('grok.js: Dispatching input event.'); // Added log
                 // Dispatch an input event to notify any listeners
                 const inputEvent = new Event('input', { bubbles: true });
                 promptInput.dispatchEvent(inputEvent);
                 console.log('grok.js: Input event dispatched.'); // Added log
                 ...
                 console.log("grok.js: Change event dispatched for textarea.");


                 console.log("grok.js: Prompt text inputted and input event dispatched."); // This log confirms these steps
                 ...
                 console.log(`grok.js: inputPromptAndSend: Waiting for send button with selector: "${sendButtonSelector}"`); // Added log
                 ...
                 if (sendButton) {
                     console.log("grok.js: Send button element found:", sendButton); // Log the element
                     console.log(`grok.js: Found send button with selector "${sendButtonSelector}". Attempting to click.`); // This log confirms button was found
                     ...
                     try {
                         ...
                         if (isFirstLeftPromptSent) {
                             chrome.runtime.sendMessage({
                                 action: "leftTabSendButtonClicked", // Use the generic 'leftTabSendButtonClicked' for Grok when it's the left tab
                                 // No need for tabId here, background knows the sender tab
                             })
                             .then(() => console.log("grok.js: Sent 'leftTabSendButtonClicked' message to background (First prompt)."))
                             .catch(error => console.error("grok.js: Error sending 'leftTabSendButtonClicked' message (First prompt):", error));

                             isFirstLeftPromptSent = false; // Set the flag to false after sending the first time
                         } else {
                             console.log("grok.js: Skipping sending 'leftTabSendButtonClicked' message (Not the first prompt).");
                         }
                         // --- End Send message ---

                         // Simulate a click event on the button
                         sendButton.click();
                         console.log("grok.js: Send button clicked."); // This log confirms the click was attempted

                         // Set the processing flag to true (already set at the start of message handler)
                         console.log("grok.js: isProcessingGrokPrompt remains true.");

                         // --- Start the GrokButtonStateObserver ---
                         // Start the observer *after* clicking the send button to monitor response state.
                         if (grokButtonObserver && !grokButtonObserver.observing) { // Only start if not already observing
                              grokButtonObserver.start();
                              console.log("grok.js: Started GrokButtonStateObserver.");
                         } else if (grokButtonObserver) {
                              console.log("grok.js: GrokButtonStateObserver is already observing.");
                         } else {
                              console.warn("grok.js: GrokButtonStateObserver instance not found. Cannot start observer.");
                           }

                         // No need to send a response back here, the response handling logic will signal completion.

                     } catch (clickError) {
                         console.error(`grok.js: Error clicking send button with selector "${sendButtonSelector}" on Grok tab:`, clickError);
                         // If clicking fails, reset the processing flag
                         isProcessingGrokPrompt = false;
                         console.log("grok.js: isProcessingGrokPrompt set to false due to click error.");
                          // Optionally, send an error message back to the background
                         /*
                         chrome.runtime.sendMessage({
                             action: "grokResponseFinished", // Use the finished action to signal an issue
                     ...
                     console.warn(`grok.js: Send button with selector "${sendButtonSelector}" not found or not enabled on Grok site after delay. Cannot send prompt.`);
                     ...
                     if (grokButtonObserver && !grokButtonObserver.observing) { // Only start if not already observing
                         grokButtonObserver.start();
                         console.log("grok.js: Started GrokButtonStateObserver even though send button not found/enabled.");
                     } else if (grokButtonObserver) {
                         console.log("grok.js: GrokButtonStateObserver is already observing.");
                     } else {
                         console.warn("grok.js: GrokButtonStateObserver instance not found. Cannot start observer.");
                     ...
                     isProcessingGrokPrompt = false;
                     console.log("grok.js: isProcessingGrokPrompt set to false due to send button not found.");
                 ...
                 console.error(`grok.js: Error inputting prompt text into element with selector "${promptInputSelector}" on Grok tab:`, inputError);
                 // If input fails, reset the processing flag
                 isProcessingGrokPrompt = false;
                 console.log("grok.js: isProcessingGrokPrompt set to false due to input error.");
                  // Optionally, send an error message back to the background
                 /*
                 chrome.runtime.sendMessage({
                     action: "grokResponseFinished", // Use the finished action to signal an issue
            ...
            console.error(`grok.js: Error waiting for prompt input element with selector "${promptInputSelector}" on Grok tab:`, waitForElementError);
            // If waiting for input fails, reset the processing flag
            isProcessingGrokPrompt = false;
            console.log("grok.js: isProcessingGrokPrompt set to false due to wait for element error.");
             // Optionally, send an error message back to the background
            /*
            chrome.runtime.sendMessage({
                action: "grokResponseFinished", // Use the finished action to signal an issue
    ...
    console.log('grok.js: inputPromptAndSend function declared.');
    ...
    chrome.runtime.onMessage.addListener(async (request, sender, sendResponse) => { // Made listener async
        console.log("grok.js: Grok content script received message:", request);
        ...
        if (request.action === "saveLeftPrompt") {
            console.log("grok.js: Received 'saveLeftPrompt' action.");
            ...
            if (isDualPromptingActive) {
                 ...
                 console.log("grok.js: Saved initial prompt for dual prompting:", savedLeftPrompt);
                 // DO NOT automatically process the prompt here. Wait for the ChatGPT response.
                 sendResponse({ status: "Prompt saved for dual prompting on Grok tab" });
            } else {
                 console.log("grok.js: Dual prompting is NOT active. Skipping saving initial prompt.");
        ...
        if (request.action === "receiveChatGPTResponse") {
            ...
            if (!isDualPromptingActive) {
                 console.log("grok.js: Dual prompting is NOT active. Skipping processing ChatGPT response.");
                 sendResponse({ status: "Dual prompting inactive, skipping response processing" });
                 return true; // Indicate async response
            }

            if (isProcessingGrokPrompt) {
                console.warn("grok.js: Still processing previous prompt, deferring new response from other AI");
                sendResponse({ status: "Busy processing previous prompt" });
                return true; // Indicate async response
            }

            console.log("grok.js: Received 'receiveChatGPTResponse' action.");
            isProcessingGrokPrompt = true; // Set processing flag
            const chatGPTResponse = request.chatGPTResponse; // Assuming the response text is in chatGPTResponse property
            console.log("grok.js: Received ChatGPT response:", chatGPTResponse);

            // --- Combine the saved initial Grok prompt and the ChatGPT response ---
            // Use the saved initial prompt as the base
            let combinedPrompt = savedLeftPrompt;
            ...
            if (chatGPTResponse && chatGPTResponse.trim() !== '') { // Check if ChatGPT response is not empty
                ...
                combinedPrompt = (combinedPrompt ? combinedPrompt + '\n\n' : '') + `--- Response from other AI ---\n${chatGPTResponse}`; // Ensure newline if initial prompt exists
                 console.log("grok.js: Combined prompt with ChatGPT response.");
            } else {
                console.warn("grok.js: Received null, empty, or whitespace-only ChatGPT response. Only sending the initial prompt.");
                 console.log("grok.js: Using only the initial prompt.");
            ...
            savedLeftPrompt = null; // Use null instead of empty string for consistency
            console.log("grok.js: Cleared savedLeftPrompt after combining.");
            ...
            // isProcessingGrokPrompt is reset within inputPromptAndSend if it fails
            // It remains true if inputPromptAndSend successfully clicks the button,
            // and is reset by handleGrokButtonStateChange when the response finishes.

            sendResponse({ status: "ChatGPT response received, combined prompt inputted and send button clicked on Grok tab" });
            return true; // Indicate async response
            ...
            console.log(`grok.js: Received updateDualPromptingStatus: isDualPromptingActive set to ${isDualPromptingActive}`);
    ...
    function notifyBackgroundScriptReady() {
        console.log("grok.js: Attempting to send 'contentScriptReady' message to background from Grok tab.");
        try {
            chrome.runtime.sendMessage({ action: "contentScriptReady" });
            console.log("grok.js: Sent 'contentScriptReady' message to background from Grok tab.");
        } catch (error) {
            console.error("grok.js: Error sending 'contentScriptReady' message from Grok tab:", error);
            // This error often means the background script is not available (e.g. extension updated or crashed)
        }
    }
    console.log('grok.js: notifyBackgroundScriptReady function declared.');

    // Function to send a ping message to the background script to keep it alive (Defined within the guarded block)
    function sendPing() {
        // console.log("grok.js: Attempting to send ping to background from Grok tab."); // Keep pings quiet unless debugging
        try {
            chrome.runtime.sendMessage({ action: "ping" });
            // console.log("grok.js: Sent ping to background from Grok tab."); // Keep pings quiet unless debugging
        } catch (error) {
            // This error often means the background script is not available (e.g. extension updated or crashed)
            // console.error("grok.js: Error sending ping from Grok tab:", error); // Keep quiet unless debugging
            // If ping fails, the background script's periodic check should detect the unresponsiveness
        }
    }
    console.log('grok.js: sendPing function declared.');
    ...
    try {
        ...
        console.log('grok.js: DOMContentLoaded/load listeners set up.');
    } catch (error) {
        console.error("grok.js: Error setting up DOMContentLoaded/load listeners:", error);
    ...
    try {
        ...
        console.log('grok.js: Custom CSS injected into Grok tab.');
    } catch (error) {
        console.error("grok.js: Error injecting custom CSS into Grok tab:", error);
    }
    ...
    function injectAudioIframe() {
        ...
        if (document.getElementById('audio-player-iframe')) {
            console.log('grok.js: Audio iframe already exists.');
            return;
        }
        ...
        try {
            ...
            console.log('grok.js: 🎶 Audio iframe injected!');
        } catch (error) {
            console.error("grok.js: Error injecting audio iframe:", error);
        }
    }
    console.log('grok.js: injectAudioIframe function declared.');
    ...
    try {
        ...
        console.error("grok.js: Error setting up iframe injection listeners:", error);
